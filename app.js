...
app.get("/tweets/:tweetId", authenticateToken, async (request, response) => {
  ...
  const tweetsQuery = `SELECT * FROM tweet WHERE tweeId=${tweetId};`;
  ...
  if (
    ...
    const getTweetDetailsQuery = `SELECT tweet,COUNT(DISTINCT(like.like_id)) AS likes,
        ...
        like ON tweet.tweet_id=like.like_id INNER JOIN reply.tweet_id=tweet.tweet_id WHERE 
...
INNER JOIN like ON like.tweetId=tweet.tweetId WHERE tweet.tweetId=${tweetId} AND 
follower.follower_user_id=${user_id};`;
...
app.get(
  ...
  async (request, response) => {
    ...
    const { user_id, name, username, gender } = payload;
    console.log(name, tweetId);
    const getRepliedUsersQuery = `SELECT * FROM follower INNER JOIN tweet ON tweet.user_id=follower.following_user_id 
    INNER JOIN reply ON reply.tweet_id=tweet.tweet_id WHERE tweet.tweet_id=${tweetId} AND 
    follower.follower_user_id=${user_id};`;
...
app.get("user/tweets", authenticateToken, async (request, response) => {
  ...
  const getTweetsDetailsQuery = `SELECT tweet.tweet AS tweet,COUNT(DISTINCT(like.like_id)) AS likes,
  COUNT(DISTINCT(reply.reply_id)) AS replies,tweet.date_time AS dateTime FROM user INNER JOIN tweet ON 
  user.user_id=tweet.user_id INNER JOIN like ON like.tweet_id=tweet.tweet_id INNER JOIN reply ON 
  reply.tweet_id=tweet.tweet_id WHERE user.user_id=${user_id} GROUP BY tweet.tweet_id;`;
